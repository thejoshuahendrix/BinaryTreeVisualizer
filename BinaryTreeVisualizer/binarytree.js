...
class Node {
  ...
  draw() {
    ...
    if (this.left !== null) {
      ctx.strokeStyle = this.left.color;
      ctx.beginPath();
      ctx.moveTo(this.x, this.y + 8); // draw line from center of node
      ctx.lineTo(this.left.x + 3, this.left.y + 3); // draw line above and right of node
      ctx.stroke();
      this.left.draw();
    }
    ctx.beginPath();
    ...
    ctx.fillText(this.value, this.x - 10, this.y);
  }
  insert(value) {
    if (Number(this.value) <= value) {
      if (this.right == null) {
